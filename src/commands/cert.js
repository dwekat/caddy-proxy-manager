  ...
  .action(async (domain, options) => {
    try {
      ...
      if (!domainExistsInConfig(domain)) {
        console.error(
          chalk.red(
            `Domain ${domain} does not exist in configuration. Add it first with 'add' command.`
          )
        );
        process.exit(1);
      }
      ...
      if (options.cert && options.key) {
        if (!fs.existsSync(options.cert)) {
          console.error(
            chalk.red(`Certificate file not found: ${options.cert}`)
          );
          process.exit(1);
        }
        if (!fs.existsSync(options.key)) {
          console.error(
            chalk.red(`Certificate key file not found: ${options.key}`)
          );
          process.exit(1);
        }
      ...
      // Generate a new certificate using mkcert (default behavior)
        ...
        console.log(
          chalk.green(`Generated new certificate for ${domain} using mkcert`)
        );
      }
      ...
      await reloadCaddy();
      console.log(
        chalk.green(`Caddy reloaded with new certificate configuration`)
      );
    } catch (error) {
      console.error(
        chalk.red(`Failed to update certificate: ${error.message}`)
      );
      process.exit(1);
    }
  });
