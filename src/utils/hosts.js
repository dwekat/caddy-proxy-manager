    ...
    execSudo(checkCommand)
      .then((result) => {
        ...
        if (exists) {
          ...
          console.log(
            chalk.blue(`Domain ${domain} does not exist in hosts file.`)
          );
          resolve(false);
        }
      })
      .catch((error) => {
        console.error(
          chalk.red(`Error checking domain existence: ${error.message}`)
        );
...
export function addDomainToHosts(domain) {
  return ensureHostsBlock()
    ...
    .then((exists) => {
      if (exists) {
        console.log(
          chalk.yellow(`Domain ${domain} already exists in hosts file.`)
        );
...
' ${HOSTS_FILE}
        ...
        execSudo(combinedCommand)
          ...
          .catch((error) => {
            console.error(
              chalk.red(`Failed to add domain to hosts file: ${error}`)
            );
            reject(error);
          });
      });
